      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            className="fixed inset-0 m-auto w-full max-w-lg h-fit glass-dark border border-red-500/20 rounded-[2.5rem] z-[110] overflow-hidden shadow-2xl"
          >
            <div className="p-8 md:p-10">
              <div className="flex justify-between items-start mb-8">
                <div>
                  ...
                  <p className="text-white/40 text-sm font-medium">Connect directly with our HGP operators.</p>
                </div>
              ...
              <div className="space-y-4">
                {SUPPORT_CHANNELS.map((channel, i) => (
                  <motion.a
                    ...
                    className={`group block glass p-6 rounded-3xl border border-white/5 ${channel.borderColor} ${channel.color} transition-all shadow-xl ${channel.glowColor}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-5">
                        <div className="w-12 h-12 glass rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform bg-white/5">
                          {channel.icon}
                        </div>
                        <div>
                          <h4 className="text-lg font-bold text-white group-hover:text-inherit transition-colors">{channel.name}</h4>
                          <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest">{channel.handle}</p>
                        </div>
                      </div>
              ...
              <div className="mt-10 pt-8 border-t border-white/5 text-center">
                <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.3em]">Operational 24/7 • Crimson Shield Verified</p>
              </div>
            </div>
          </motion.div>
        </>
